...
if __name__ == "__main__":
    ...
    val_img_bb = h_val.create_dataset("image_bb", (len(val_imgids), num_fixed_boxes, 4), "f")
    ...
    with open(infile, "r") as tsv_in_file:
        ...
        for item in tqdm(reader):
            ...
            bboxes = np.frombuffer(
                base64.decodebytes(bytes(item["boxes"], encoding="utf8")), dtype=np.float32
            ...
            if image_id in train_imgids:
                ...
                train_img_features[train_counter, :, :] = np.frombuffer(
                    base64.decodebytes(bytes(item["features"], encoding="utf8")), dtype=np.float32
                ).reshape((item["num_boxes"], -1))
